...
public class BackDOrderDaoImpl implements BackDOrderDao{
	@Override
	public List<Order> selectAllDOrder(int pageSize, int pageNo) {
		Connection conn = DBHelper.getConnection();
		String sql = "SELECT goods_name,goods_color,order_num,order_msum,users_region,users_address,users_name,users_phone,dorder.users_num,order_id,order_date,stock.goods_size FROM dorder INNER JOIN stock  on dorder.stock_id=stock.stock_id INNER JOIN goods  ON stock.goods_id=goods.goods_id INNER JOIN address  ON dorder.address_id=address.address_id limit ?,?";
	...
	public int selectMaxPageNo(int pageSize) {
		Connection conn = DBHelper.getConnection();
		String sql = "select count(*) from dorder";
		int total = 0;
		try {
			PreparedStatement ps =conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
